...
После развертывания, запускается из консоли командой `vagrant up`. 
...
По умолчанию он ищет сайты в каталоге `D:\htdocs`, это можно изменить в файле `Params.rb`.
...
Альтернативой этому является использование утилиты WinHosts, которая просматривает папку `D:\htdocs` и дописывает все нужные домены в файл `hosts`.
